...
$$\text{Final Score} = (S_{\text{Tech}} \times 0.5625) + (S_{\text{Consensus}} \times 0.25) + (S_{\text{Risk}} \times 0.1875)$$

> **2026-09-03 개정 — $S_{\text{AI}}$ 를 점수에서 뺐다.** LLM 은 가격을 예측하지 못한다.
> 지표를 넣고 "상승 확률 81%"를 물으면 그럴듯한 숫자를 만들어낼 뿐이고 근거가 없다.
> 검증할 수 없는 값을 점수의 20% 로 쓰면 점수 전체가 검증 불가능해진다.
> **AI 는 "왜 이 신호가 나왔는지 설명"에만 쓴다.** 위 비율은 원래 구성비(45/20/20/15)에서
> AI 몫을 빼고 남은 셋을 비례 재분배한 것이다.

- **$S_{\text{Tech}}$ (기술적 지표 점수 — 100점 만점, 거래량 가중 평균 지표 기준)**:

  > **2026-09-04 개정 — 배점표 v3 (Step 16 재설계).** v2 배점표에는 세 가지 문제가
  > 겹쳐 있었다. 고친 것은 셋 다 **구조**이고, 임계값(RSI 30/70, ADX 25, 거래량 30%)은
  > 하나도 바꾸지 않았다. 아래 "왜 데이터에 맞추지 않았는가" 참고.
  >
  > **① 비대칭.** 가점 +110 / 감점 -30 이라 중립 시장의 점수가 10 이 되었고, §3.2의
  > 60/40 임계값과 맞추려고 기준점 40 을 얹는 보정이 필요했다. MACD 는 골든크로스에
  > +15 를 주면서 데드크로스는 **계산조차 하지 않았고**, MA 정배열 +15 에 역배열은
  > 0점이었다. 즉 하락 근거를 점수로 표현할 방법이 사실상 없었다.
  > → 모든 항목을 0 중심 대칭으로 만들고 **기준점을 50** 으로 올렸다. 이제 중립이
  > 보정 없이 정확히 50 이다.
  >
  > **② 중복.** RSI·스토캐스틱·CCI 의 실측 상관은 r = 0.76 ~ 0.84 로 사실상 같은
  > 지표인데 합쳐서 40점을 차지했다. "여러 근거가 일치한다"는 착각이 점수로 굳는
  > 구조였다. → **모멘텀 축을 RSI 하나(10점)로 합쳤다.**
  >
  > **③ 검증되지 않은 배점.** 호가 불균형 10점은 백테스트 표본 9,443건 중 **0건**에서만
  > 켜졌다 — 백테스트에 호가 데이터가 없어 한 번도 검증된 적이 없다. → 점수에서 뺐다.

  | 항목 | 배점 | 판정 |
  |---|---|---|
  | 기준점 | **50** | 중립 |
  | 모멘텀 (RSI) | **±10** | 30 이하 +10 / 70 이상 −10 / 50 이 0, 선형 |
  | 추세 배열 (MA) | **±15** | 정배열 +15 / 역배열 −15 / 혼조 0 |
  | 추세 강도 (ADX) | **±10** | ADX 25 이상이면 MA 배열 방향으로 ±10 |
  | MACD 교차 | **±10** | 골든 +10 / 데드 −10 |
  | 볼린저 위치 | **±5** | 하단 이탈 후 복귀 +5 / 상단 돌파 지속 −5 |
  | 거래량 급증 | **±5** | 30% 이상 급증을 MA 배열 방향으로 ±5 |
  | 스토캐스틱 · CCI | 0 | 참고 표시 (RSI 와 중복) |
  | 호가 불균형 | 0 | 참고 표시 (백테스트 검증 불가) |
  | VWAP 이격 | 0 | 참고 표시 (RSI 와 r = 0.80) |

  - 합산 후 `min(max(sum, 0), 100)` 으로 clamp
  - 점수에서 뺀 항목도 **0점짜리 내역으로 남긴다** — 화면에서 사라지면 왜 뺐는지도
    같이 사라지고, 다시 넣을지 판단할 근거(값 자체)가 쌓이지 않는다.

  **왜 실측에 "맞춰" 쓰지 않았는가.** 같은 진단을 세 시기(최근 14일 / 28~42일 전 /
  56~70일 전, 각 5,700여 표본)에서 돌렸더니 **개별 항목의 예측력 부호가 시기마다
  뒤집혔다.** RSI 는 최근 구간에서 과매도가 가장 나빴지만(−0.029%) 56~70일 전 구간에서는
  과매도가 가장 좋았다(+0.591%). 추세 계열도 최근 구간에서만 뚜렷했고 나머지 두
  구간에서는 차이가 사라졌다. 한 구간의 수치에 맞춰 배점을 돌리면 다음 국면에서 정확히
  반대로 작동하는 표가 된다. 그래서 v3 은 **구조만 고치고 숫자는 건드리지 않았다.**
