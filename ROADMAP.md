...
> 9/2에 Step 0과 Step 1-a를 함께 끝내 **9/3 이후 일정을 하루씩 당겼다. 종료일 9/19 → 9/18.**
...
| 9/3 (목) | Step 1-b — Bybit/Coinbase 추가 + 지표 4종(Bollinger/Stochastic/ADX/CCI) + 글로벌 가중 평균 |
| 9/4 (금) | Step 2-a — Consensus 계산 + RuleEngine 재작성 |
| 9/5 (토) | Step 2-b — OpenAI 연동 + `ai_signals` 저장/publish |
| 9/6 (일) | Step 3-a — 유저 웹훅 발급/재발급 API (PHP) |
| 9/7 (월) | Step 3-b — 멀티테넌트 웹훅 수신 엔드포인트 (Python) + 유저 격리 테스트 |
| 9/8 (화) | Step 4-a — 백테스팅 엔진(신호검증용) |
| 9/9 (수) | Step 4-b — 백테스팅 엔진(업비트 실전용, 수수료/슬리피지) |
| 9/10 (목) | Step 5 — 실거래 안전장치·매매 실행(업비트 전용) |
| 9/11 (금) | Step 6-a — 엔드포인트 5종 + 공통 에러 포맷 |
| 9/12 (토) | Step 6-b — JWT 인증 + Rate Limit |
| 9/13 (일) | Step 7 — 성과 추적 스케줄러 |
| 9/14 (월) | Step 8 — 테스트 · 로깅 · 알림 훅 |
| 9/15 (화) | Step 9-a — 신호 목록·상세 화면 (합의율 표시 포함) |
| 9/16 (수) | Step 9-b — 점수 추이 차트 + 법적 고지 |
| 9/17 (목) | Step 10-a — 언어팩 인프라 (lang/*.json, locale 설정, 유저 언어 변경) |
| 9/18 (금) | Step 10-b — AI 문구 실시간 번역/캐싱 + 법적 고지 다국어 검수 반영 |