...
> 9/1에 Step 1-a·1-b를 함께 끝내 **9/2 이후 일정을 하루씩 당겼다**. 종료일 9/11 → 9/10.

| 날짜 | 진행 |
|---|---|
| 9/1 (화) | ~~Step 1-a — Upbit WebSocket 수집 + Redis 캐싱~~ 완료 |
| 9/1 (화) | ~~Step 1-b — 지표 계산(RSI/MACD/MA/호가불균형) + 재접속 백오프~~ 완료 (9/2 예정분 선행) |
| 9/2 (수) | Step 2 — RuleEngine + OpenAI + ai_signals 저장 |
| 9/3 (목) | Step 3 — 백테스팅 엔진 |
| 9/4 (금) | Step 4 — 실거래 안전장치·매매 실행 |
| 9/5 (토) | Step 5-a — 엔드포인트 5종 + 공통 에러 포맷 |
| 9/6 (일) | Step 5-b — JWT 인증 + Rate Limit |
| 9/7 (월) | Step 6 — 성과 추적 스케줄러 |
| 9/8 (화) | Step 7 — 테스트 · 로깅 · 알림 훅 |
| 9/9 (수) | Step 8-a — 신호 목록·상세 화면 |
| 9/10 (목) | Step 8-b — 점수 추이 차트 + 법적 고지 |
